...
public:
    ...
    // getters
    // Return the exact current move counter used by tests
    virtual int  getCurrentMove() const { return numMoves; }
    // White moves on even numMoves; black on odd
    ...
    const Piece* getPiecePtr(int col, int row) const { return board[col][row]; }
