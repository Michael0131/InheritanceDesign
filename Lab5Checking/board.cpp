...
void Board::move(const Move& move)
{
    ...
    if (move.isEnPassant() && pSrc && pSrc->getType() == PAWN)
    {
        ...
        // Replace captured pawn with a Space (no delete, no reallocation)
    ...
    // --------------------------------------------
    if (move.isPromotion() && pSrc->getType() == PAWN)
    {
        // clear destination piece (if any)
        delete pDst; pDst = nullptr;

        // place a Queen of the mover's color
        board[dst.getCol()][dst.getRow()] = new Queen(dst, pSrc->isWhite());

        // clear source
        delete pSrc; pSrc = nullptr;
        board[src.getCol()][src.getRow()] = new Space(src.getCol(), src.getRow());
